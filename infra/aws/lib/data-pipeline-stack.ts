...
export class LcaDataPipelineStack extends Stack {
  constructor(scope: Construct, id: string, props: DataPipelineStackProps) {
    ...
    userData.addCommands(
      ...
      `# Barrier lives in a repo script (keeps user-data under EC2's 16KB limit; it runs`,
      `# the queue-depth helper from apps/harvester, where bullmq/ioredis resolve).`,
      `REGION=$REGION SNS_TOPIC=${notifyTopic.topicArn} RELEASE=$RELEASE \\`,
      `  bash /opt/lca/infra/aws/scripts/burst-barrier.sh $BARRIER_QUEUES`,
      ``,
