...
export class LcaDataPipelineStack extends Stack {
  constructor(scope: Construct, id: string, props: DataPipelineStackProps) {
    ...
    userData.addCommands(
      ...
      `# still terminates it, so cost stays bounded. The "no new data" path exits 0`,
      `# cleanly and does not trip this trap.`,
      `trap 'aws sns publish --topic-arn ${notifyTopic.topicArn} --subject "[LCA] burst FAILED (\${RELEASE:-pending})" --message "Burst $INSTANCE_ID failed at line $LINENO. See CloudWatch /lca/burst/userdata. Box stays up for debugging; 36h watchdog will terminate it." || true' ERR`,
      ``,
      ...
      ``,
      `# ----- 36h cost watchdog -----`,
      `# If the operator never clicks "Shut down", self-destruct after 36h so a`,
      `# forgotten c7g can't bill indefinitely. The operator's Shut-down button`,
      `# (and a successful "no new data" early-exit) terminate sooner. Reuses the`,
      `# teardown script (removes DNS + SNS + terminate).`,
      `( sleep 129600; bash /opt/lca/infra/aws/scripts/ec2-teardown.sh ) \\`,
