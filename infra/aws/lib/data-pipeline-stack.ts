...
export class LcaDataPipelineStack extends Stack {
  constructor(scope: Construct, id: string, props: DataPipelineStackProps) {
    ...
    userData.addCommands(
      ...
      `  HARVEST_START_YEAR=\${HARVEST_START_YEAR:-2010} \\`,
      ...
      `fi`,
      ``,
      `# Deferred NLP? (per-run DeferNlp instance tag) — wait only for INGEST at the`,
      `# barrier; the NLP worker keeps draining nlp-tasks in the BACKGROUND on this`,
      `# kept-alive review box, and a later Rebuild-preview picks up the normalised`,
      `# data. Default = wait for both queues (normal quarterly path).`,
      `DEFER_NLP=$(aws ec2 describe-tags --region $REGION --filters "Name=resource-id,Values=$INSTANCE_ID" "Name=key,Values=DeferNlp" --query 'Tags[0].Value' --output text 2>/dev/null || echo false)`,
      `[ "$DEFER_NLP" = "None" ] && DEFER_NLP=false`,
      `echo "defer NLP: $DEFER_NLP"`,
      `# Always drain INGEST first so every row is in lca_records. (Barrier lives in a`,
      `# repo script — keeps user-data under EC2's 16KB limit; runs the queue-depth`,
      `# helper from apps/harvester where bullmq/ioredis resolve.)`,
      `REGION=$REGION SNS_TOPIC=${notifyTopic.topicArn} RELEASE=$RELEASE \\`,
      `  bash /opt/lca/infra/aws/scripts/burst-barrier.sh ingest-tasks`,
      `# Normal (quarterly) path: GUARANTEE + VERIFY NLP coverage for the just-ingested`,
      `# year(s) — ingest's per-batch enqueue is best-effort, so sweep -> drain nlp ->`,
      `# assert (no false-completeness). DeferNlp leaves NLP to the decoupled processor.`,
      `if [ "$DEFER_NLP" != "true" ]; then`,
      `  NOTIFY_TOPIC=${notifyTopic.topicArn} REGION=$REGION RELEASE=$RELEASE LOCAL_FILES_DIR=$LOCAL_FILES_DIR \\`,
      `    bash /opt/lca/infra/aws/scripts/burst-nlp-coverage.sh`,
      `fi`,
      ``,
      `# Ingested into PG — archive the inbox files out of the way.`,
      `archive_inbox`,
      ``,
      `# Candidate build + operator review env — extracted to a repo script (user-data`,
      `# is capped at 16KB). CDK-resolved names/ARNs passed as env; a non-zero exit`,
      `# propagates to the ERR trap above (failure SNS + box stays up for debugging).`,
      `NOTIFY_TOPIC=${notifyTopic.topicArn} \\`,
      `  LLM_SECRET=${shared.llmApiKeySecret.secretName} \\`,
      `  LCADB_BUCKET=${shared.lcaDbBucket.bucketName} \\`,
      `  ECR_REPO=${shared.lambdaImageRepo.repositoryName} \\`,
      `  PGSNAP_BUCKET=${shared.pgSnapshotBucket.bucketName} \\`,
      `  OPERATOR_PW_SECRET=${shared.operatorPasswordSecret.secretName} \\`,
      `  SESSION_SECRET_ID=${shared.sessionSecret.secretName} \\`,
      `  CF_TOKEN_SECRET=${shared.cloudflareTokenSecret.secretName} \\`,
      `  REGION=$REGION RELEASE=$RELEASE INSTANCE_ID=$INSTANCE_ID SUMMARY_JSON="$SUMMARY_JSON" \\`,
      `  bash /opt/lca/infra/aws/scripts/burst-finalize.sh`,
    );
