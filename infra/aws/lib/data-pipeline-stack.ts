...
export class LcaDataPipelineStack extends Stack {
  constructor(scope: Construct, id: string, props: DataPipelineStackProps) {
    ...
    userData.addCommands(
      ...
      `archive_inbox`,
      ``,
      `# Refresh the analytics matviews from PG via the db container's psql (the host`,
      `# has none; 'analytics:refresh-views' is a root psql script, not a pnpm filter).`,
      `# Non-fatal: a stale matview shouldn't block the review env, and a later`,
      `# Rebuild-preview refreshes again once deferred NLP finishes.`,
      ...
      ``,
      `# Build the SQLite snapshot + summaries from PG. data/ is gitignored (not in the`,
      `# clone) so create it first, else SQLite can't open the new lca.db.`,
      ...
      `# --provenance/--sbom=false: Lambda rejects OCI attestation manifests (matches CI).`,
