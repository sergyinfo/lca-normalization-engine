...
### Step 8 — AWS go-live: `dev.h1b.report` LIVE ✅ *(deployed 2026-06-03)*

> **DEPLOYED.** The site serves real H-1B data from AWS at **https://dev.h1b.report**
...
> PAT populated, email subscribed to the build-notifications SNS, and a historical
> `latest.pgdump` uploaded (so the first burst restores data, not an empty DB).
...
| **Infrastructure & DevOps** | **98 %** | **`dev.h1b.report` LIVE on AWS** (§Step 8): dedicated `h1b-report` sub-account under Org "915 Solutions"; `LcaSharedStack` + `LcaServeStack` + `LcaBudgetsStack` deployed; ACM cert + Cloudflare DNS. Serving = public Lambda Function URL (OAC was broken in-account) + CloudFront origin-secret header; edge Functions for canonical-host redirect + `dev` noindex; budgets + anomaly detection live; ECR/S3 lifecycle codified (~$1/mo). Local Docker stack (pgvector, Redis, ingestor, nlp-worker, operator-ui, analytics-ui, analytics-web) still used for builds. Remaining: deploy `LcaDataPipelineStack` (P1 burst), activate cost-allocation tags (~24h), automated tests. |
