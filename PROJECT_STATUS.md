...
### Step 8 — AWS go-live prep: dev→prod parameterization ⚠️ *(code done 2026-06-02; deploy pending operator)*
...
| **Infrastructure & DevOps** | **98 %** | Docker stack with pgvector, Redis, ingestor, nlp-worker, operator-ui, analytics-ui, analytics-web all healthy. AWS CDK (4 stacks) **parameterized for a staged dev→prod rollout** (§Step 8): per-env domain/TLS via context, static-asset S3 sync fixed, private-repo burst clone via PAT, daily DOL check. `tsc`+`cdk synth` green. Logging + tagging + budgets in place; GitHub Actions ships analytics-web end-to-end. Remaining work is operator console steps (Org sub-account, `cdk bootstrap`, ACM, Cloudflare DNS, secret population) + automated tests. |
